...
import EBarChart from "expensasaurus/components/budgets/BudgetBarChart";
...
import { getAllLists, getDoc } from "expensasaurus/shared/services/query";
...
import { calcTotalExpByCategoryBuget } from "expensasaurus/shared/utils/calculation";
...
const BudgetByIDPage = () => {
  ...
  const {
    data: budgetDurationExpenses,
    isLoading: isBDELoading,
    isSuccess: isBDESuccess,
      ...
      Object.entries(budgetByCategory).map(([key, val]) => {
        ...
        return [
          key,
          {
            budget: val,
            ...expense,
            budgetPercent: val === 0 && amount === 0 ? 0 : (amount / val) * 100,
          },
        ];
      })
    );
        ...
        <>
          <TabGroup>
            <div className="flex items-center justify-between mb-4">
              <Metric className="text-slate-600 font-thin">
            ...
            <Subtitle className="mb-4 text-slate-500">
              {data?.description}
            </Subtitle>
            <TabPanels>
              <TabPanel>
                <Subtitle className="my-6 text-slate-600">
                  Spending in categories with budget
                </Subtitle>
                <Grid numItemsSm={2} numItemsLg={3} className="gap-4">
                  {Object.entries(match().result)
                    .filter(([category, value]) => value.budget)
                    .map(([category, value], i) => {
                      ...
                      if (!value.budget && !value.transactionsCount)
            ...
            <div className="mt-6 flex flex-col sm:flex-row justify-between text-white bg-blue-700 p-6 shadow-subtle rounded-[500px] relative">
              <div
                style={{
                  width:
                    totalBudgetConsumedPercent > 100
                      ? "100%"
              ...
              <p className="z-[2] text-[14px] sm:text-[16px]">
                Budget: {formatCurrency(data?.currency, data?.amount)}
              </p>
              <p className="z-[2] text-[14px] sm:text-[16px]">
                {totalBudgetConsumedPercent.toFixed(2)}% consumed
              </p>
              <p className="z-[2] text-[14px] sm:text-[16px]">
                Spending :{" "}
                {formatCurrency(data?.currency, match().totalExpense)}
              </p>
            </div>
            <BudgetStatus
              type={
